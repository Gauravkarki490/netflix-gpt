...
const Login = () => {
  ...
  return (
    <div  >
      ...
      <form className='absolute p-12 bg-black w-3/12 my-28 mx-auto right-0 left-0 text-white bg-opacity-80 rounded-lg'>
        <h1 className='font-bold text-3xl py-4'>{isSignInForm ? "Sign In" : "Sign Up"}</h1>
        {!isSignInForm && <input type='text' placeholder='Name' className='p-4 my-4 w-full bg-gray-500 rounded-lg' />}
        ...
        {!isSignInForm && <input type='password' placeholder='Confirm Password' className='p-4 my-4 w-full bg-gray-500 rounded-lg' />}
        <button className='p-4 my-6 bg-red-600 w-full rounded-lg'>{isSignInForm ? "Sign In" : "Sign Up"}</button>
        <p className='py-4 cursor-pointer inline' onClick={toggleSignInForm}>{isSignInForm ? "New to Netflix? Sign Up Now" : "Already Registered. Sign In"}</p>
      </form>
    </div>
  )
}
